...
MUST_PASS_PATHS = [
    ...
    os.path.join('external', 'vulkancts', 'mustpass', 'master', 'vk-default-no-waivers.txt'),
...
def check_and_add_tabs(line, string_name, string_value, field_index, tab_size):
    ...
    assert \
        ...
        '{} "{}" is too long!'.format(string_name, string_value)
...
def add_amber_test(input_amber_test_file_path, vk_gl_cts):
    ...
    for must_pass_file_path in MUST_PASS_PATHS:
        add_amber_test_to_must_pass(
            amber_test_name,
            os.path.join(vk_gl_cts, must_pass_file_path),
        )
...
def main(args):
    ...
    parser.add_argument(
        'vk_gl_cts',
        help='Path to a checkout of VK-GL-CTS',
    )
