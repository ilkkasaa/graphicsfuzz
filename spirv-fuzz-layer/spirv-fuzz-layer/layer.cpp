...
void SpirvFuzzLayer::PreCallApiFunction(const char *api_name)
{
	printf("Calling %s\n", api_name); 
}

// Intercept the memory allocation calls and increment the counter
VkResult SpirvFuzzLayer::PostCallAllocateMemory(VkDevice device, const VkMemoryAllocateInfo *pAllocateInfo,
...
void SpirvFuzzLayer::PreCallFreeMemory(VkDevice device, VkDeviceMemory memory, const VkAllocationCallbacks *pAllocator) {
...
VkResult SpirvFuzzLayer::PreCallQueuePresentKHR(VkQueue queue, const VkPresentInfoKHR *pPresentInfo) {
    present_count_++;
    if (present_count_ >= display_rate) {
		...
		SpirvFuzzLayer::Information(message.str());
